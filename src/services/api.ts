...
export const api = {
  auth: {
    login: (data: { serverUrl: string; username: string; password: string }) =>
      request<{ token: string; username: string; mode: string }>('/auth/login', { method: 'POST', body: JSON.stringify(data) }),
    getDomains: () =>
      request<{ domains: string[]; mode: string }>('/auth/domains'),
    getProjects: (domain: string) =>
      request<{ projects: string[]; mode: string }>(`/auth/projects?domain=${encodeURIComponent(domain)}`),
    connectProject: (data: { domain: string; project: string }) =>
      request<{ message: string; mode: string }>('/auth/connect', { method: 'POST', body: JSON.stringify(data) }),
  ...
  user: {
    getGroups: () =>
      request<{ groups: string[] }>('/user/groups'),
